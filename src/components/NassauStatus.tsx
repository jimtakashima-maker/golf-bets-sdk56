...
// column so the numbers line up vertically across Front / Back / Overall
// instead of drifting with the name's length. "Dormie" means the trailing
// side can, at best, halve the segment from here (their deficit exactly
// matches the holes left). "Closed" means the segment is already
// mathematically decided - the deficit is bigger than the holes left, so
// the trailing side can no longer even tie it.
//
// This is the round-wide standings view only - live and actionable press
// info (the "+ Press" button, in-progress press rows) lives on the Score
// tab instead, via NassauPressPanel, since a press only makes sense in the
// context of "what hole are we on right now" for the tee group actually
// playing that matchup.
...
const SEGMENT_KEYS: Array<'front' | 'back' | 'overall'> = ['front', 'back', 'overall'];
const SEGMENT_LABELS: Record<'front' | 'back' | 'overall', string> = {
  front: 'Front',
  back: 'Back',
  overall: 'Overall',
};
...
function MatchupCard({
  ...
  return (
    <View style={[styles.matchup, divider && styles.matchupDivider]}>
      ...
      {(totalHoles === 9 ? (['front'] as const) : SEGMENT_KEYS).map((segmentKey) => {
...
// Read-only hole-by-hole detail for one Nassau matchup - each side's
...
// actually decided it.
function NassauMatchupDetailModal({
  visible,
  matchup,
  totalHoles,
  onClose,
